...
path_in = (str(image1))
#path_in = ('E:/Annie Madam/Biology/Pancreas/liver_images/fatty/thresholded_samples/' + str(image1))
#img=cv.imread(path_in)
img = Image.open(path_in)

image_type = input("Enter 'y' if the thresholded image is of manually tagged data: ")
time1 = time.time()
...
def white_segments(y_axis, x_axis, s_size):
    ...
    section_items = []
    for section_n in range(yx_total):
    ...
    y_pseudo_section = y_axis - ((y_total - 1) * s_size) # group of pixels left out whicha re unable to represent length of tile
    ...
    for section_n in range(len(segmented_areas)):
        ...
        if len(segmented_areas[section_n]) > 0:
            if (section_n + 1) % x_total == 0:
                x_length = x_pseudo_section - 1
            else:
                x_length = s_size - 1
            if int(section_n / x_total) == y_total - 1:
                y_length = y_pseudo_section - 1
            else:
                y_length = s_size - 1
            ...
            while (i < len(segmented_areas[section_n])):
                ...
                while (i0 > 1):
                    if segmented_areas[section_n][i][i0][0] < y_length:
                        break
                    else:
            ...
            while (i < len(segmented_areas[section_n])):
                i0 = len(segmented_areas[section_n][i]) - 1
                while (i0 > 1):
                    if segmented_areas[section_n][i][i0][2] == x_length:
                        ...
                        while (i1 > 1):
                            if segmented_areas[section_n][i][i1][0] < temp3 - 1:
                                break
                            elif segmented_areas[section_n][i][i1][2] == x_length:
                                temp1 = segmented_areas[section_n][i][i1][0]
                                temp3 = temp3 - 1
                            i1 = i1 - 1
    ...
    print('section_boundary, yx_total: ', len(section_boundary), yx_total)
    combine_seg_sec = []
        ...
        if len(section_boundary[section_n]) > 0:
            for j in range(1,len(section_boundary[section_n])):
                if section_boundary[section_n][j][0] == 2:
                    ...
                    # section_n - x_total
                    if (section_n - x_total) >= 0: #  indicates topmost section
                        for i in range(1,len(section_boundary[section_n - x_total])):
                            if section_boundary[section_n - x_total][i][0] == 1:
                                ...
                                while (j1 < len(section_boundary[section_n][j])):
                                    ...
                                    j1 = j1 + 1
                                break
                elif section_boundary[section_n][j][0] == 1:
                    # section_n + x_total
                    if (section_n + x_total) <= (yx_total) - 1: #  indicates bottommost section
                        for i in range(1,len(section_boundary[section_n + x_total])):
    ...
    # sequentially combine the combine_seg_sec groups (tiled image to single image)
    i1 = 0
    while (i1 < len(combine_seg_sec)):
        #print(i1, len(combine_seg_sec))
        temp1 = 0
